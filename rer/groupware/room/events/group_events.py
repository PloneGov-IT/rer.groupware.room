...
def addNotifyGroup(info_dict,event):
    ...
    group_notify_news_events="%s.notifyNewsEvents" %room_id
    group_notify_docs="%s.notifyDocs" %room_id
    user_groups=user.getGroups()
    already_signed=False
    for group in user_groups:
        if group.startswith(room_id) and not group == group_id:
            if group in [group_notify_news_events,group_notify_docs]:
                already_signed = True
            else:
    ...
    if already_signed:
        return
    group = acl_users.getGroup(group_notify_news_events)
    if not group:
        return
